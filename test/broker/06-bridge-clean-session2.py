...
def do_test(proto_ver, cs, lcs=None):
    print("Running test with cs:%s, lcs: %s and proto: %d" % (cs, lcs, proto_ver))
    if proto_ver == 4:
    ...
    try:
        ...
        print("Normal bi-dir bridging works. continuing")
        ...
        print("client A reconnected after restarting broker A at ", time.time())
        # Need to sleep long enough to be sure of a re-connection...
        time.sleep(10)  # yuck, this makes the test run for ages!
        ...
        if expect_queued_ba:
            ...
            print("not expecting message b->a_2")
